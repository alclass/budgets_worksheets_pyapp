...
import lib.indices.bls_us.report_reqstatus_in_jsondatafiles_cls as statusex
import lib.os.sufix_incrementor as osfs
...
class Fetcher:
  ...
  def process_old(self):
    ...
    if self.bool_jsonfile_is_already_present_in_folder:
      print(scrmsg)
      n_step += 1
      action_str = 'json stored file exists, dbfetch_months_n_refmonths_as_dict_bt_refmonths it.'
      scrmsg = f'Step {n_step} - {action_str}.'
      print(scrmsg)
  ...
  def process(self):
    ...
    n_step += 1
    action_str = 'save fetched json or json-dbfetch_months_n_refmonths_as_dict_bt_refmonths to its prettyprint datafile'
    scrmsg = f'Step {n_step} - {action_str}.'
    print(scrmsg)
