...
class ExchangeRateDate(Base):
  ...
  origin            = Column(String(10), default='BCB PTAX')
  ...
  def __repr__(self):
    outstr = '<ExRate %s/%s exchange rate q=%.4f on d=%s>' %(self.numerator_curr3, self.denominator_curr3, self.buyquote, self.quotesdate)
    return outstr
