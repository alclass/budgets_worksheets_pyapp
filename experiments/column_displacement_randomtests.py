...
In words, it checks a logical deduction that the implementation of cell_letters as a number system
...
Here is the explanation.
  ...
  2) because the move operation is an addition or subtraction to the cell_letters,
    at some moments, this operation will fall into a zero: example: Z+1 must be shifted one (1) to AA.
    When this happen, a plus-one operation becomes plus-two.
...
There are two ways to find that below. The simpler one has an output from random numbers such as:
...
Thus said, backnumber is the "thermometer" that tells us the the move-operation is not commutative,
  ie, x,y,z cells-moved to the right may not return to its origin with an equal x,y,z cells-moved to the left.

Solution: we don't have it yet, but if we can map, one by one, the cell-letters set to the integer set,
  then, it'll work. Lexicographic algorithms may be one way to look up into the matter.
  Anyways, it's an open issue now.
...
def generate_random_numbers(innerlooprun): # innerlooprun = innerloopruns[decimalplace]
  ongoing_conv_letters = 'a'; baknumber = 0
  total = 0; parcelcounter = 0; positnumcount = 0; negatnumcount = 0
